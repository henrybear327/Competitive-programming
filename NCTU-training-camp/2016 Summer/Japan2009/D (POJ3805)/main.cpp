...
bool intersect(seg a, seg b)
{
    ...
    if(overlap(a, b) == true) {
        ...
        // bitch man, EPS...
        if( d - (dist(a.x, a.y) + dist(b.x, b.y)) > EPS  ) // d > dist(a.x, a.y) + dist(b.x, b.y)
            return false;
        return true;
    }

    // bitch man.... Equal sign..
    if(ccw(a.x, a.y, b.x) * ccw(a.x, a.y, b.y) <= 0 && ccw(b.x, b.y, a.x) * ccw(b.x, b.y, a.y) <= 0)
        return true;
    return false;
}
...
vector<pt> halfHull(vector<pt>& points)
{
    ...
    for(int i = 0; i < (int)points.size(); i++) {
        while((int)res.size() >= 2
            ...
            res.pop_back(); // res.size() - 2 can't be assign before size() >= 2 check, bitch
...
bool completelyInside(vector<pt>& outer, vector<pt>& inner)
{
    ...
    for(int i = 0; i < (int)inner.size(); i++) {
        ...
        for(int j = 0; j < (int)outer.size(); j++) {
            ...
            if( (b.x.x * slope + offset == b.x.y) || (b.y.x * slope + offset == b.y.y)) {// on-line
                i--;
                break;
            }
...
int main()
{
    ...
    while(scanf("%d %d", &n, &m) == 2 && (n || m)) {
        ...
        // check if any white convex hull line segment intersects with black's convex hull line segment
        // check if any black convex hull line segment intersects with white's convex hull line segment
        bool ok = true;
        for(int i = 0; i < (int)blackHull.size() && ok; i++) {
            for(int j = 0; j < (int)whiteHull.size() && ok; j++) {
                seg a = seg(blackHull[i % (int)blackHull.size()], blackHull[(i + 1) % (int)blackHull.size()]);
                seg b = seg(whiteHull[j % (int)whiteHull.size()], whiteHull[(j + 1) % (int)whiteHull.size()]);
#if DEBUG == 1
        ...
        // else, print NO
        if(ok && n >= 3 && m >= 3 && blackInWhite == false && whiteInBlack == false)
            printf("YES\n");
        else if(ok && n < 3 && m < 3) {
            printf("YES\n");
        } else
